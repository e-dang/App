...
import {AccessTokenPayload, createPasswordResetToken, dateToSeconds, passwordIsValid, RefreshTokenPayload} from "@auth";
import {AuthenticationError, InvalidTokenError, SignInError, UserWithEmailAlreadyExistsError} from "@errors";
import nodemailer from "nodemailer";
import {randomUUID} from "crypto";
import {mocked} from "jest-mock";
import {createUser, extractCookies, decode} from "./utils";
...
describe("auth apis", () => {
  ...
  describe("/signup", () => {
    ...
    test("returns an accessToken with userId in payload", async () => {
      ...
      const payload = decode<AccessTokenPayload>(res.body.data.accessToken as string);
    ...
    test("returns a refreshToken in header with userId in payload", async () => {
      ...
      const payload = decode<RefreshTokenPayload>(extractCookies(res.headers).rt.value);
      expect(payload.userId).not.toBeUndefined();
      const user = await User.findOne({id: payload.userId});
      expect(user.email).toBe(email);
      expect(user.name).toBe(name);
  ...
  describe("/signin", () => {
    ...
    test("updates last login time for the user", async () => {
      ...
      await supertest(app).post(url).send(signInData);
  ...
  describe("/signout", () => {
    ...
    test("increments user's tokenVersion on success", async () => {
      await supertest(app).post(url).set("Authorization", `Token ${accessToken}`).send();
  ...
  describe("/password/reset/confirm", () => {
    ...
    test("returns 400 status code when no user with the specified userId exists", async () => {
      const res = await supertest(app).post(url).send({userId: randomUUID(), token, newPassword, confirmPassword});
