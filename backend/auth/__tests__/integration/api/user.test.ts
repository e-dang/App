...
import {createUser, decode} from "./utils";
...
describe("user apis", () => {
  ...
  describe("GET /user", () => {
    ...
    test("returns 401 status code if access token has been tampered with", async () => {
      const res = await supertest(app).get(url).set("Authorization", `Token ${accessToken}adiajwdijo`).send();
  ...
  describe("PATCH /user", () => {
    ...
    test("returns 401 status code if access token has been tampered with", async () => {
      const res = await supertest(app).patch(url).set("Authorization", `Token ${accessToken}adiwojaiodj`).send({
        name: newName,
      });
  ...
  describe("DELETE /user", () => {
    ...
    test("sets isActive field on user to false on success", async () => {
      await supertest(app).delete(url).set("Authorization", `Token ${accessToken}`).send();
    ...
    test("returns 401 status code if access token has been tampered with", async () => {
      const res = await supertest(app).delete(url).set("Authorization", `Token ${accessToken}awdioahjdiu29`).send();
